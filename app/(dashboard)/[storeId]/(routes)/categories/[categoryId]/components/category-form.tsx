...
// Reusable Form value
type CatygoryFormValues = z.infer<typeof formSchema>;
...
const CategoryForm: React.FC<CategoryFormProps> = ({ initialData, billboards }) => {
  ...
  // Making form schema useing zod
  const form = useForm<CatygoryFormValues>({
  ...
  // Handle form submit
  const onSubmit = async (data: CatygoryFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(
